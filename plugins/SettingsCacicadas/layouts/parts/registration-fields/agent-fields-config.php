...
?>
<div ng-if="field.config.entityField == '@terms:funcao'">
    <?php foreach ($funcao_options as $field_name) : ?>
        <div ng-if='field.config.entityField === "<?= $field_name ?>"'>
            ...
            <textarea ng-model="field.fieldOptions" ng-init='field.fieldOptions = field.fieldOptions || data.taxonomies.funcao.terms.join("\n")' placeholder="<?php \MapasCulturais\i::esc_attr_e("Opções de seleção"); ?>"></textarea>
        </div>
    <?php endforeach ?>
</div>